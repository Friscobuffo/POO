...
public class Statistiche {
	...
	public Map<Formica, Integer> raccoltoPerFormica(Set<Cibo> raccolto) {
		...
		for(Cibo c : raccolto) {
			Formica f = c.getRaccoglitrice();
			Integer i = formica2raccolto.get(f);
			if (i == null) {
				i = 0;
			}
			formica2raccolto.put(f, ++i);
		}
	...
	public Map<Class<? extends Formica>, Set<Cibo>> raccoltoPerTipoDiFormica(Set<Cibo> raccolto) {
		Map<Class<? extends Formica>, Set<Cibo>> tipo2cibo = new HashMap<>();
		for (Cibo c : raccolto) {
			Formica f = c.getRaccoglitrice();
			Set<Cibo> cibo = tipo2cibo.get(f.getClass());
			if (cibo==null) {
				cibo = new HashSet<>();
				tipo2cibo.put(f.getClass(), cibo);
			}
			cibo.add(c);
		}
		return tipo2cibo;
	...
	public List<Class<? extends Formica>> ordinaStrategieDiRaccolta(final Map<Class<? extends Formica>, Set<Cibo>> c2c) {
		List<Class<? extends Formica>> result = new LinkedList<>(c2c.keySet());
		
		result.sort(new Comparator<Class<? extends Formica>>() {
			@Override
			public int compare(Class<? extends Formica> c1, Class<? extends Formica> c2) {
				return c2c.get(c2).size() - c2c.get(c1).size();
			}
		});
